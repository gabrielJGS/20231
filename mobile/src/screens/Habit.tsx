...
export function Habit() {
  ...
  async function fetchHabits() {
    try {
      ...
      console.log(error);
      Alert.alert("Ops!", "Não foi possível buscar as informações do hábito");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <View className="flex-1 bg-background px-8 pt-16">
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        {isDateInPast && (
          <Text className="text-white mt-10 text-center">Você não pode atualizar um hábitos de uma data passada</Text>
        )}
      </ScrollView>
    </View>
  );
